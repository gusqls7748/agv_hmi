...
void NetworkServer::handlePostCommand(QTcpSocket *socket, const QByteArray &body) {
    ...
    // 1. manual_mode를 command보다 먼저 처리합니다.
    //    따라서 {"manual_mode":true,"command":"forward"} 같은 한 번의 요청도 정상 처리됩니다.
    bool manualModeWasSpecified = jsonReq.contains("manual_mode");
    if (manualModeWasSpecified) {
        bool newMode = false;
        ...
        if (jsonReq["manual_mode"].isBool()) {
            ...
            QString modeStr = jsonReq["manual_mode"].toString().trimmed().toLower();
        ...
        // 수동 모드를 끄면 현재 속도를 즉시 0으로 만듭니다.
        if (!m_isManualMode && m_topicPublisher) {
            m_topicPublisher->stopManualDrive();
        }
        updated = true;
    }
    ...
    if (jsonReq.contains("destination")) {
        ...
        bool accepted = m_topicPublisher && m_topicPublisher->publishGoal(dest.toStdString());

        if (!accepted) {
            qWarning() << "[NetworkServer] Rejected destination:" << dest;
            QJsonObject res;
            res["result"] = "fail";
            res["error"] = "등록되지 않은 목적지이거나 Topic Publisher가 연결되지 않았습니다: " + dest;
            sendHttpResponse(socket, 400, res);
            return;
        }
        updated = true;
    ...
    if (jsonReq.contains("command")) {
        ...
        if (cmd == "cancel") {
            // Nav2 Action을 직접 취소하지 않습니다.
            // 현재는 안전을 위해 cmd_vel을 0으로 만드는 것만 수행합니다.
            if (m_topicPublisher) {
                m_topicPublisher->stopManualDrive();
            }
            updated = true;
        } else if (manualDirections.contains(cmd)) {
            // 기존 클라이언트가 {"command":"forward"}만 보내더라도 동작하도록
            // manual_mode가 명시되지 않은 경우 자동으로 수동 모드에 진입합니다.
            // 명시적으로 {"manual_mode":false,"command":"forward"}를 보낸 경우에는 거부합니다.
            if (!m_isManualMode && manualModeWasSpecified) {
                qWarning() << "[NetworkServer] Manual command rejected because manual_mode=false:" << cmd;
                QJsonObject res;
                res["result"] = "fail";
                res["error"] = "manual_mode가 false입니다. 수동 조종 전에 manual_mode:true를 보내주세요.";
            ...
            if (!m_isManualMode) {
                m_isManualMode = true;
                emit manualModeChanged(m_isManualMode);
                qDebug() << "[NetworkServer] Manual Mode auto-enabled by command";
            }

            if (m_topicPublisher) {
                bool accepted = m_topicPublisher->manualDrive(cmd.toStdString());
                if (!accepted) {
                    QJsonObject res;
                    res["result"] = "fail";
                    res["error"] = "수동 주행 명령을 처리하지 못했습니다: " + cmd;
                    sendHttpResponse(socket, 400, res);
                    return;
                }
            }
